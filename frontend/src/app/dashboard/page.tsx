...
import {
  ...
  GlowButton,
  SkeletonLoader
} from 'src/components/MotionComponents';
    ...
    <div className="flex-1 flex flex-col min-h-screen bg-[var(--background)]">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-6 py-10 sm:px-8 space-y-12 animate-fade-in">
        ...
        <AnimatePresence>
          {error && (
            <motion.div 
              ...
              exit={{ opacity: 0, y: -15 }}
              className="flex items-center gap-3 p-4 rounded-xl bg-red-500/5 border border-red-500/10 text-red-400 text-xs font-medium shadow-sm"
            >
        ...
        <MotionContainer className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {/* Total Sessions */}
          <MotionItem>
            <AnimatedCard className="flex items-center justify-between" glowColor="rgba(139, 92, 246, 0.05)">
              <div className="space-y-1.5">
                <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Total Sessions</p>
                <h2 className="text-3xl font-black text-white tracking-tight">
                  <AnimatedCounter value={user?.total_sessions || 0} />
                </h2>
              </div>
              <div className="p-3 bg-white/[0.02] border border-white/[0.04] text-zinc-400 rounded-xl group-hover:text-violet-400 transition-colors">
          ...
          <MotionItem>
            <AnimatedCard className="flex items-center justify-between" glowColor="rgba(139, 92, 246, 0.05)">
              <div className="space-y-1.5">
                ...
                <h2 className="text-3xl font-black text-white tracking-tight">
                  <AnimatedCounter value={user?.total_mocks || 0} />
                </h2>
              </div>
              <div className="p-3 bg-white/[0.02] border border-white/[0.04] text-zinc-400 rounded-xl group-hover:text-violet-400 transition-colors">
          ...
          {/* Average Score */}
          <MotionItem>
            <AnimatedCard className="flex items-center justify-between" glowColor="rgba(0, 245, 212, 0.05)">
              <div className="space-y-1.5">
                <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Average Score</p>
                <h2 className="text-3xl font-black text-white tracking-tight">
                  {user?.avg_mock_score ? (
                    <AnimatedCounter value={Math.round(user.avg_mock_score)} suffix="%" />
                  ) : (
                    'N/A'
                  )}
                </h2>
              </div>
              <div className="p-3 bg-white/[0.02] border border-white/[0.04] text-zinc-400 rounded-xl group-hover:text-teal-400 transition-colors">
          ...
          {/* Best Score */}
          <MotionItem>
            <AnimatedCard className="flex items-center justify-between" glowColor="rgba(16, 185, 129, 0.05)">
              <div className="space-y-1.5">
                ...
                <h2 className="text-3xl font-black text-emerald-400 tracking-tight">
                  {user?.best_mock_score ? (
                    <AnimatedCounter value={Math.round(user.best_mock_score)} suffix="%" />
                  ) : (
                    'N/A'
                  )}
                </h2>
              </div>
              <div className="p-3 bg-emerald-500/5 border border-emerald-500/10 text-emerald-400 rounded-xl group-hover:bg-emerald-500/10 transition-colors">
                <Award className="w-5 h-5" strokeWidth={1.5} />
              </div>
            </AnimatedCard>
          </MotionItem>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 pt-4 border-t border-white/[0.02]">
          <div className="space-y-1.5">
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map(i => (
              <SkeletonLoader key={i} rows={3} height="h-56" />
            ))}
          </div>
        ) : sessions.length === 0 ? (
          <motion.div 
            ...
            className="premium-card p-16 text-center max-w-3xl mx-auto flex flex-col items-center justify-center border-dashed border-white/[0.06] hover:border-violet-500/20"
          >
            ...
            <AnimatePresence>
              {sessions.map((session) => (
                <MotionItem key={session.id}>
                  <AnimatedCard className="h-full flex flex-col justify-between" glowColor="rgba(139, 92, 246, 0.03)">
                    <div>
                      <div className="flex items-start justify-between gap-4">
                        ...
                        {session.status === 'ready' && (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider bg-emerald-500/5 border border-emerald-500/10 text-emerald-400">
                            <span className="w-1 h-1 rounded-full bg-emerald-400 animate-pulse" />
                            Ready
                          </span>
                        )}
                        {session.status === 'processing' && (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider bg-violet-500/5 border border-violet-500/10 text-violet-400">
                            <Loader2 className="w-2.5 h-2.5 animate-spin" strokeWidth={1.5} />
                            Analyzing
                          </span>
                        )}
                        {session.status === 'failed' && (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider bg-red-500/5 border border-red-500/10 text-red-400">
                      ...
                      <p className="text-zinc-400 text-xs mt-4 flex items-center gap-2 truncate bg-[#0c0c0e]/40 border border-white/[0.03] p-3 rounded-xl">
                    ...
                    <div className="mt-6 flex flex-col min-[380px]:flex-row items-stretch min-[380px]:items-center justify-between gap-3 pt-4 border-t border-white/[0.04]">
                      {session.status === 'ready' ? (
                        <div className="flex-1 flex flex-col min-[380px]:flex-row gap-3">
                          <Link
                            ...
                            className="flex-1 inline-flex justify-center items-center py-2.5 px-3 bg-zinc-950/60 border border-white/[0.05] hover:bg-zinc-900 rounded-xl text-xs font-bold uppercase tracking-wider text-zinc-400 hover:text-zinc-200 transition-colors cursor-pointer text-center"
                          >
                            Practice Bank
                          </Link>
                          <Link
                            href={`/sessions/${session.id}/mock/setup`}
                            className="flex-1 inline-flex justify-center items-center gap-1.5 py-2.5 px-3 bg-violet-600 hover:bg-violet-500 rounded-xl text-xs font-bold uppercase tracking-wider text-white transition-all shadow-md shadow-violet-500/10 cursor-pointer text-center hover:-translate-y-0.5 active:scale-95"
                          >
                      ...
                      <motion.button
                        ...
                        className="p-2.5 text-zinc-500 hover:text-red-400 bg-transparent hover:bg-red-500/5 rounded-xl border border-transparent hover:border-red-500/10 transition-colors cursor-pointer flex-shrink-0 disabled:opacity-50 self-end min-[380px]:self-auto"
                      >
